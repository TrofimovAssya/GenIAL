...
def get_prob_set(mer_tab,chrom):
    ...
    for i in mer_tab:
        ...
        prob = np.log2(float(count)/float(total))
        if not isinf(prob):
            psum+=prob
    if not psum == 0 and not isinf(psum):
        return prob*np.log2(float(prior))
    else:
        return None
...
def predict_BCF(read,n):
    ...
    for k in jfDB.keys():
        ...
        if not p == None and not isinf(p):
            chromprob[k] = p

    if not len(chromprob)==0:
        print chromprob
        return max(chromprob,key = chromprob.get)
    else:
        return None
